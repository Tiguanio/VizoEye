...
- **Три режима**:
    ...
    - *Голосовой вопрос*: Диалог с ИИ по фото. Поддерживает 
       контекст (помнит последние 3 вопроса), что позволяет задавать 
       уточняющие вопросы.
...
- **Clean Architecture:** Разделение на слои (data, 
     domain, ui) для поддерживаемости кода.
- **Manual DI:** Отказ от тяжелых фреймворков (Hilt/Koin)
     в пользу легковесного `AppContainer`.

## ⚡ Производительность
    Проект ориентирован на минимальную задержку.
     
## 🔒 Безопасность
    API-ключи защищены с использованием Android Keystore / 
